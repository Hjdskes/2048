...
public class GameScreenTest {
	...
	public void setUp() {
		...
		screen = new GameScreen(stage, grid, scores, overlays, buttons);
	...
	public void testCreate() {
		...
		verify(stage).addActor(overlays);
		verify(stage).addActor(buttons);
	}
	...
	/**
	 * Tests if all required methods are called on screen.resize(... , ...).
	 */
	@Test
	public void testResize() {
		screen.resize(0, 0);
		verify(viewPort).update(anyInt(), anyInt(), anyBoolean());
	}
}
